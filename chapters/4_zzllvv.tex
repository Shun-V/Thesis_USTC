...
\begin{itemize}
    \item Section~\ref{sec:samples} describes the data and Monte Carlo (MC) simulation samples used to model the signal and background processes.
    \item Section~\ref{sec:event_selection} details the reconstruction of physics objects and the selection criteria defining the Signal Regions (SR).
    \item Section~\ref{sec:background} discusses the estimation of major backgrounds, particularly the dominant $WZ$ and non-resonant backgrounds, using data-driven control regions.
    \item Section~\ref{sec:systematics} outlines the sources of experimental and theoretical systematic uncertainties.
    \item Section~\ref{sec:unfolding} presents the unfolding procedure used to correct the detector-level distributions to the particle level.
    \item Section~\ref{sec:dnn} describes the optimization strategies, including the use of Deep Neural Networks (DNN), to enhance the separation between signal and background.
    \item Section~\ref{sec:results_llvv} reports the measured integrated and differential cross-sections for the inclusive $ZZ$ production and the constraints on aTGCs.
