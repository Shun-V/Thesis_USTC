...
\item Parton-shower and hadronisation uncertainties are evaluated using eigenvector variations of the Pythia~8 A14 tune, where available (e.g., for top-pair production). For signal and diboson samples generated with \textsc{Sherpa}, alternative truth-level samples are used. Typical parton-shower uncertainties range from 1\% to 4\% for different MC samples.
...
The experimental uncertainties include contributions from lepton reconstruction and identification, trigger efficiencies, and the energy/momentum scale and resolution. Systematic uncertainties related to jets, $E_\mathrm{T}^\mathrm{miss}$, flavour-tagging, pile-up, and luminosity are also considered. The impact of each major experimental systematic uncertainty on the signal is reported as ``up'' ($+1\sigma$) and ``down'' ($-1\sigma$) variations. Overall, the total experimental uncertainties remain below 2.5\% for all signal samples.

%%%%%%%%%%%%%%%%
For signal processes, Tables~\ref{tab:syst-uncertainties-ee} and \ref{tab:syst-uncertainties-mm} show the impact of the experimental uncertainties on the $qq\to ZH$ signal in the $ee$ and $\mu\mu$ channels, respectively. Tables~\ref{tab:syst-uncertainties-MonoZ-ee} and \ref{tab:syst-uncertainties-MonoZ-mm} summarize the individual impacts for the benchmark axial-vector signal in the $ee$ and $\mu\mu$ channels, respectively. The full mapping between the physical descriptions used in these tables and the specific ATLAS internal parameter names is provided in Appendix~\ref{subsec:app_syst}.

The systematic uncertainty on the total yield for all reconstructed simplified models and the 2HDM+a model signals is shown in Figure~\ref{fig:MonoZ-syst} (a) and (b). Binned variations for each source are used in the limit-setting procedure.
...
\bottomrule
\end{tabular}
\caption{\rm Impact of the 20 dominant experimental uncertainties on the $qq\to ZH$ signal in the $ee$ channel. The total positive (negative) uncertainty is calculated by summing the largest positive (negative) variations for each source in quadrature, including smaller sources that are not shown.}
...
\bottomrule
\end{tabular}
\caption{\rm Impact of the 20 dominant experimental uncertainties on the $qq\to ZH$ signal in the $\mu\mu$ channel. The total positive (negative) uncertainty is calculated by summing the largest positive (negative) variations for each source in quadrature, including smaller sources that are not shown.}
...
For some non-dominant backgrounds in the control regions (CRs), systematic uncertainties are assigned based on those evaluated in the signal region (SR). Specifically, normalization uncertainties on $Z$+jets are set to 38\% in the $e\mu$ and $3\ell$ CRs, while $WZ$ uncertainties in the $e\mu$ region are set to 9\%. The $ZZ \to \ell\ell\nu\nu$ uncertainties in the $e\mu$ and $3\ell$ CRs are set to 6.6\%, and 8.8\% for $ZZ \to 4\ell$. Uncertainties of the non-resonant processes in the $3\ell$ CR are set to 14\%. For triboson and $t\bar{t}V$ processes, conservative uncertainties of 50\% and 20\% are assumed, respectively.