...
The total experimental systematic uncertainties on the expected yield for all reconstructed simplified models and the 2HDM+$a$ model signals have been evaluated. Across the wide range of signal mass hypotheses, the impact of the experimental uncertainties remains relatively uniform. For the simplified model signal samples, the total systematic uncertainty typically ranges between 2\% and 3\%, with maximum values around 5\% for specific parameter choices. Similarly, for the 2HDM+$a$ samples, the total systematic uncertainty predominantly falls within the range of 2\% to 4\%, peaking at approximately 6\%. Because of the large number of signal points and the uniformity of the uncertainties, these values are summarized here rather than presented individually. In the final limit-setting procedure, binned variations for each systematic source are implemented consistently.
