...
Signal and background predictions are simultaneously fitted to the observed data in the signal region (SR) and control regions (CRs) using the \textsc{HistFitter} framework. The behavior of the nuisance parameters (NPs) included in the combined fit was carefully examined. All nuisance parameters are found to be well-behaved, with their post-fit values remaining fully consistent with the pre-fit expectations within one standard deviation ($\pm 1\sigma$). No significant asymmetrical pulls or unexpected over-constraints are observed for any of the systematic uncertainties. Since the fit does not reveal any anomalous behavior in the uncertainty modeling, the individual NP pulls are omitted for brevity.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[angle=-90,width=0.9\linewidth]{figures/ch6-DM/fig_07.pdf}
