...
This search utilizes the full ATLAS Run 2 dataset ($\sqrt{s}=13~\text{TeV}$), corresponding to an integrated luminosity of $139~\text{fb}^{-1}$ corresponded to the integrated luminosity before 2023. 

The analysis performed prior to 2023 utilized the preliminary Run~2 luminosity calibration, which yielded $139~\mathrm{fb}^{-1}$ with an uncertainty of approximately $1.7\%$. In 2023, the ATLAS collaboration published the final luminosity determination, which refined the absolute luminosity scale using van der Meer beam separation scans and applied comprehensive long-term stability corrections across multiple luminosity-sensitive detectors. This final calibration updated the total integrated luminosity to $140 ~\mathrm{fb}^{-1}$, halving the relative uncertainty to $0.83\%$. Consequently, the respective integrated luminosity values used in this thesis are strictly consistent with the ATLAS recommendations at the time each analysis was finalized.
