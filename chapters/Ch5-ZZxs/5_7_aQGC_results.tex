...
The measurement of $\text{ZZ}jj$ production is sensitive to VBS processes, offering a unique probe into $\text{ZZWW}$ and $\text{ZZZZ}$ quartic vertices. As introduced in Section~\ref{subsec:theory_aqgc}, the search for Anomalous Quartic Gauge Couplings (aQGCs) is interpreted within the SMEFT framework. This analysis specifically evaluates the dimension-8 transversal operators $\text{T}0$ through $\text{T}9$ ($\mathcal{O}_{\text{T}i}$), extracting limits on their associated Wilson coefficients $f_{\text{T}i}/\Lambda^4$ ($i = 0, 1, \ldots, 9$).
% -------------------------------------------
