...
\label{subsec:diff-xs}

Differential cross-sections are measured in the fiducial phase spaces by counting data events in each bin of the relevant observables, subtracting the expected background contributions, and correcting for detector effects. Detector-level distributions are unfolded to the stable-particle level using an iterative Bayesian method~\cite{DAgostini:1995}, which accounts for resolution effects, reconstruction inefficiencies, and bin migrations. Response matrices derived from simulated signal samples encode the probability of an event generated in a given particle-level bin being reconstructed in a corresponding detector-level bin. 

The number of iterations, typically between two and five depending on the observable, is optimized to balance statistical uncertainty and unfolding stability. The binning is defined using purity and efficiency criteria to ensure reliable unfolding and preserve the distribution shapes. The final bin of each observable is open-ended and extends to the kinematic limit, with no separate overflow bin. Validation studies using nominal and alternative samples show no significant bias. 
The kinematic variables used for differential cross-section measurements are summarized in Table~\ref{tab:diff-var}.
\begin{table}[!hbtp]
    \centering
%    \scriptsize
  ...
  \textbf{Observable description} &  \textbf{Observable} &  \textbf{SR} \\
     ...
     Number of jets                   &   $N_\text{jets}$ & $ZZ$ only\\  
...
Inclusive $ZZ$ production (see, in particular, the diagram in Figure~\ref{fig:zz-prod}c) provides a key channel for probing anomalous neutral triple gauge couplings (aNTGCs). In the SM, the $s$-channel diagram involving the neutral $ZZZ$ or $ZZ\gamma$ vertex is absent, making this process particularly sensitive to possible deviations from SM expectations.
...
\begin{figure}
    ...
    \caption{Distribution of the transverse momentum of the dilepton system, $p_\text{T}^\text{Z}$, including data and SM predictions.
...
% The most general anomalous $ZZV$ ($V=Z,\gamma$) vertex function for on-shell $\text{Z}$ bosons is given by~\cite{Hagiwara:1986vm}:
% \begin{equation}
...
% \label{eqn:vtx_func}
% \end{equation}
% where $P$, $q_1$, and $q_2$ denote the momenta of the $V$ boson and the two on-shell $\text{Z}$ bosons, respectively. The anomalous couplings $f_4^V$ and $f_5^V$ correspond to CP-violating and CP-conserving interactions.
...
% --- Modified: Theory moved to Chapter 2 ---
As detailed in Section~\ref{subsec:theory_atgc}, limits on aTGC parameters are derived using two complementary theoretical frameworks: the vertex-function (VF) formalism and the Standard Model Effective Field Theory (SMEFT). 

For the VF formalism, the anomalous couplings $f_4^V$ and $f_5^V$ ($V = \text{Z}, \gamma$) corresponding to CP-violating and CP-conserving interactions are probed. Within the SMEFT framework, this analysis specifically investigates the dimension-eight operators $\mathcal{O}_{G+}$, $\mathcal{O}_{G-}$, $\mathcal{O}_{\widetilde{B}W}$, $\mathcal{O}_{BB}$, $\mathcal{O}_{BW}$, and $\mathcal{O}_{WW}$. The constraints are placed on their corresponding Wilson coefficients $C_i/\Lambda^4$, with the new-physics scale fixed to $\Lambda = 1~\text{TeV}$.
% -------------------------------------------
