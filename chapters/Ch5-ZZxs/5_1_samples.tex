...
This section details the datasets and simulated samples employed in the analysis. The measurement is performed on the full proton-proton ($pp$) collision dataset recorded by the ATLAS experiment during Run~2 of the LHC at a centre-of-mass energy of $\sqrt{s} = 13$~TeV.
...
The MC predicted cross-sections calculated with these generators are given in Table~\ref{tab:sherpa-xs} for both $ZZ$ and $ZZjj$ phase space.
Details of these samples used in the analyses are provided in Table~\ref{tab:ZZ_signal_samples}.
...
\end{tabularx}
\caption{\rm Summary of Monte Carlo samples used to model the SM $ZZ \to \ell^+\ell^-\nu\bar{\nu}$ signal components. Cross-sections ($\sigma$) are provided by the generator, and k-factors are applied to correct for higher-order predictions.}
...
The Run II pp collision data collection by the ATLAS experiment starts from 2015 and ends in 2018. This analysis uses the full Run II pp collision data using Release 21 reconstruction, which passes the final Good Run List (GRL) released by the Data Quality group for 2015-2018. 
...
\bottomrule
\end{tabular}
\caption{\rm Good Run Lists (GRLs) used in the analysis.}
\label{tab:grls}
\end{table}
...
\begin{tabular}{ l p{6.5cm} p{5cm} }
    ...
    \hline
\end{tabular}
\caption{\rm Single-lepton triggers used for this analysis by data period and lepton flavor.}
