...
\begin{figure}[!htbp]
    ...
    \caption{Performance profiles showing the data identification efficiency ($\epsilon_{\text{id}}$) for electrons, evaluated against the electron's transverse energy $E_{\text{T}}$ (a) and its pseudorapidity $\eta$ (b). The efficiencies are measured using the tag-and-probe method in $Z \to ee$ and $J/\psi \to ee$ events from $37.1~\text{fb}^{-1}$ of $pp$ collision data at $\sqrt{s} = 13~\text{TeV}$, corresponding to an average pile-up of $\langle\mu\rangle \approx 24$~\cite{ATLAS-Egamma-2019}.}
...
\begin{figure}[!htbp]
    ...
    \caption{Visual breakdown of the detector components utilized for isolating muons. The classification of a muon candidate depends directly on which tracking sub-systems contribute to its reconstruction. \textit{Combined} muons successfully merge trajectory data from both the Muon Spectrometer and the Inner Detector. \textit{Standalone} muons are built exclusively from Muon Spectrometer hits. \textit{Segment-Tagged} candidates pair an Inner Detector track with just one localized track segment in the Muon Spectrometer. Finally, \textit{Calorimeter-Tagged} muons rely on an Inner Detector trajectory combined with a distinct minimum-ionizing energy signature deposited within the calorimeter volume\cite{ATLAS-Muon-Reco-ID-2021}.}
    \label{fig:mu-ID}
\end{figure}
...
\begin{figure}[!htbp]
    ...
    \caption{Muon reconstruction and identification performance scaling mapped as a function of $\eta$ (a) and $p_{\text{T}}$ (b)\cite{ATLAS-Muon-Reco-ID-2021}.}
    \label{fig:mu-ID2}
\end{figure}
...
\begin{figure}[!htbp]
    ...
    \caption{The fractional energy resolution for standard jets, plotted against the transverse momentum $p_{\text{T}}$. These performance curves are shown for (a) the pseudorapidity interval $0.2 < |\eta| < 0.7$ and (b) a specific target of $\eta = 0.2$\cite{ATLAS-Jet-JER-2021}.}
    \label{fig:JER}
\end{figure}
...
\begin{figure}[!htbp]
    ...
    \caption{Vectorial derivation of the missing transverse energy, illustrating how it balances against the combined transverse momenta of all successfully reconstructed jets and leptons\cite{JETM-2020-03}.}
    \label{fig:met}
\end{figure}
