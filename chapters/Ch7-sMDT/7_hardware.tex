...
The components of a precision sMDT tube and the overall tube geometry are shown in Figures~\ref{fig:sMDT-tube}(a) and (b), respectively. A critical component is the wire locator, known as the \textit{twister}, which holds the anode wire at the center of the tube with a precision better than $10~\mu\text{m}$. The precision cylinder on the end-plug serves as the reference for the wire locator and is measured during chamber construction to determine the precise position of the wire.
...
The sMDT drift tube design and parameters are described in detail in Ref.~\cite{ATLAS-TDR-026}. Table~\ref{tab:parameters} lists the major tube parameters.
    \begin{table}[htbp]
        \centering
        \begin{tabular}{l|l}
             \hline
            ...
            Wire tension & $350 \pm 15~\text{g}$ \\
            Gas mixture & $\text{Ar}:\text{CO}_2$ (93:7) \\
            Gas pressure & 3 bar (abs.) \\
            Gas leak rate limit & $< 1 \times 10^{-8}~\text{mbar}\cdot\text{cm}^3/\text{s}$ per tube\\
            Gas gain & $2 \times 10^4$ \\
            Wire potential & 2730 V \\
%            Maximum drift time & 180 ns \\
%            Wire positioning accuracy & $10~\mu\text{m}$ r.m.s. \\
...
The second station used in tube construction is the tube crimping station, where the thin wall of the tube is deformed (crimped) to compress against the O-rings on the end-plugs and form a gas-tight seal. The crimping head is a custom-built rotary device that gradually forces three roller heads radially inward while rotating around the tube. The side view and end view of the rotator setup are shown in Figures~\ref{fig:sMDT-tubeswage}(a) and (b), respectively.
\begin{figure}[!htbp]
    \centering
        \subfloat[]{
...
%
During the test, one end of the tube is sealed with a brass cap and an O-ring, while the other end is screwed into the cap of the vacuum vessel, allowing the tube to be pressurized with helium at 3~bar. The constructed tube is required to be leak-tight with an upper leak-rate limit of $1 \times 10^{-8}~\text{mbar}\cdot\text{cm}^3/\text{s}$ at a tube pressure of 3~bar (absolute).
\begin{figure}[!htbp]
    \centering
        \subfloat[]{
...
The tube length and the wire tension are measured using the station shown in Figure~\ref{fig:sMDT-stations}(b). A tube is placed in the V-shaped aluminum bar with its midpoint inside a U-shaped magnet, and a linear digital scale is used for the length measurement. 
%
...
\]
For the nominal sMDT parameters ($L = 1597~\text{mm}$, $d = 50~\mu\text{m}$, $\rho = 19.7~\text{g/cm}^3$, $g = 9.81~\text{m/s}^2$), the first resonant frequency of 93.3~Hz corresponds to a wire tension of 350~g. The resonant frequency is extracted using a custom \textsc{LabVIEW} program that applies a fast Fourier transform (FFT) to determine $f_1$ and thus the wire tension.
...
\end{figure}
Figure~\ref{fig:sMDT-chamber-granite} shows a reconstructed sMDT chamber still held by the precision construction combs at both ends of the chamber on the large flat granite table, and the automatic glue machine, which was parked at the end of the granite table.
...
$5.00^{+0.00}_{-0.01}~\text{mm}$, concentric with the tube center (and thus the wire) within $5~\mu\text{m}$ rms. This surface is held between two half-holes formed by adjacent stacked comb plates.
%
...
$5.00^{+0.01}_{-0.00}~\text{mm}$ at the interfaces between neighboring plates.
\begin{figure}[!htbp]
    \centering
    \subfloat[]{
...
\includegraphics[width=0.57\linewidth]{figures/ch7-sMDT/measure-jigging.jpeg}}
    \caption{Measuring the precision combs using the computing-aid FARO-arm. (a) Setup of the measurement coordinates, and (b) measurement of the comb pitch by the author.}
    \label{fig:sMDT-base-plates_meas}
\end{figure}

Figure~\ref{fig:sMDT-base-plates_meas} shows the comb measurements performed using a coordinate-measuring FARO-arm, which determines three-dimensional points with a precision better than $10~\mu\text{m}$ after calibration. The measured wire-pitch distributions for both comb sets are shown in Figure~\ref{fig:sMDT-wirepitch}. The mean wire pitch agrees with the design value, with an RMS of about $4~\mu\text{m}$.
\begin{figure}[!htbp]
    \centering
...
%%%%%%%%%%%%%%%%
\noindent\textbf{2) Automatic glue machine} \\
A large gantry-mounted, computer-controlled glue-dispensing system (see Figure~\ref{fig:sMDT-chamber-granite}) was designed and built at the University of Michigan. The system controls the three-dimensional motion of the dispensing tip and the piston drive of the epoxy cartridge. The gantry is constructed from $7.6~\text{cm} \times 15.2~\text{cm}$ aluminum extrusions mounted on rail carriages that move directly on the granite table surface.

The two rails are aligned to be parallel within $7~\mu\text{rad}$. Motion along the $X$ direction is driven by a toothed rail on one side of the granite table, allowing the gantry to travel along the table length with a precision of 0.1~mm. The linear rails and carriages are regularly cleaned and lubricated to minimize friction.
...
\includegraphics[width=0.37\linewidth]{figures/ch7-sMDT/middle-comb.png}}
    \caption{Setup of base combs: 
    (a) The two precision base end-combs are connected to two sidebars by bolts indicated by the white/red arrows. The yellow/red arrows point to the hold-down clamps of the base jigging;
    (b) The inner base-combs are connected to the two sidebars. }
...
%
The inner base combs, shown in Figure~\ref{fig:sMDT-base-plates}(b), are also connected to the cross-plates and ensure tube straightness during gluing. They are aligned along the tube direction (yellow dashed lines in Figure~\ref{fig:sMDT-base-plates}(b)) and in height to better than $50~\mu\text{m}$, measured with the FARO-arm.
 
The squareness of the base-comb assembly was checked with the FARO-arm by measuring the diagonals defined by precision half-cylinders placed in comb notches 4 and 67. The two diagonals differ by $15~\mu\text{m}$, and the angle between the comb assembly and the long base plates deviates by at most $0.0018^\circ$ ($0.03~\text{mrad}$) from $90^\circ$, as shown in Figure~\ref{fig:sMDT-base-measurement}.
\begin{figure}[!htbp]
    \centering
...
The final standard deviation (RMS) of the comb flatness is below $10~\mu\text{m}$ for both base combs.
...
To accurately establish the exact 3D locations of the AP and B-field platforms within this defined coordinate system, specific physical dimensions linked to designated fiducial marks are probed across three orthogonal faces of each structure. Every individual length recorded by the FARO-arm is subsequently contrasted with its theoretical blueprint specification, enabling a precise extraction of the spatial deviations.

Figure~\ref{fig:measure-platform}(b) shows the measured AP-platform position offsets in $x$, $y$, and $z$ with respect to the nominal values. All AP platforms are positioned well within the required maximum deviation of $200~\mu\text{m}$ from the nominal position. Similar results are obtained for the CCC and B-field sensor platforms.
...
Twelve temperature sensors with readout cables are glued to the chamber surface: six on the bottom surface and six on the top tube layer. Thermal contact between the sensors and tubes is ensured using thermal epoxy paste. All sensor cables are routed to the chamber top surface through a hole in the RO-side support structure (Figure~\ref{fig:sMDT-rotation-cart}(b)). Cables for the in-plane alignment system control and readout are also routed along the top surface and connected to a readout panel mounted on the RO-side support structure.

A perforated foil is wrapped around each end of the chamber to provide a common ground connection from the tube walls to the RO and HV hedgehog cards. This foil forms the internal wall of the Faraday cage enclosing the electronics. Grounding pins are screwed into the grounding anchors installed during gluing and tightened against the foil. These pins are installed after the gas system and before the hedgehog cards are mounted.
...
The sMDT gas distribution system uses molded plastic fittings, shown in Figure~\ref{fig:sMDT-gas-installation}(a), which connects in sets of four to supply gas in series to four tubes arranged vertically across the four tube layers of a multilayer (ML).
...
\[
1\times 10^{-5}~\text{mbar}\cdot\text{L/s} \times (2N_{\text{tube}}),
\]
where $2N_{\text{tube}}$ denotes the total number of endplugs included in the measurement.

The gas volume of an sMDT tube is approximately $V_{\text{tube}} \approx 250~\text{cm}^3$. Therefore, the volume of one multilayer (ML) is
\[
V_{\text{ML}} = N_{\text{tube}} \times 250~\text{cm}^3.
\]
Using this relation, the leak-rate limit can be converted to a corresponding limit on the chamber pressure drop during measurements. For an sMDT multilayer, this corresponds to a maximum pressure drop of $0.288~\text{mbar/hour}$.
...
\]
Here $P_i$, $P_f$, $T_i$, and $T_f$ are the initial and final pressure and temperature, and $T_{\text{ref}} = 293.15~\text{K}$ is the reference temperature.
...
The final chamber performance is evaluated using cosmic-ray tests. For stable chamber operation, the grounding scheme follows a ``star'' design (see Figure~\ref{fig:grounding-chamber}) to avoid ground loops and minimize electronic noise.
\begin{figure}[!htbp]
    \centering
...
\end{figure}
All tube walls are electrically connected to grounding foils that cover the tube ends and are connected to the Faraday cages. External structures, including the gas bars, support structure, and chamber side panels, are electrically isolated from the tube walls. These components are instead connected in a star configuration to the foil ground, as illustrated in the schematic in the center of Figure~\ref{fig:grounding-chamber}, and the pictures from (a) to (d) show the connections of the grounding cables between gas bar, support frame, and chamber side-panel.

The schematic of the electrical connections to a drift tube is shown in Figure~\ref{fig:Elx-chamber}(a). For an sMDT tube, the $383~\Omega$ resistor on the HV side shown in the figure is replaced by a $330~\Omega$ resistor.
...
The noise rate of each tube is measured in a series of runs at different readout thresholds with both HV off and HV on (at the nominal operating voltage of $+2730~\text{V}$). 
...
\end{equation}
where $t_{\text{window}} = 1.55~\mu\text{s}$ is the TDC readout time window, $N_{\text{hits}}$ is the number of hits recorded in a channel during the test interval $\Delta t$, and
...
The leading and trailing edges of the TDC distribution are fitted with Fermi–Dirac functions to determine the time offset ($T_0$) and the maximum drift time ($T_{\text{max}}$). 
...
\[
r_{\text{drift}} - r_{\text{fit}},
\]
where $r_{\text{drift}}$ is the measured drift radius and $r_{\text{fit}}$ is the radius predicted by the fitted track.
...
A residual distribution is fitted with a double Gaussian function with a common mean, and the single wire resolution is defined as:
\begin{equation}
\sigma_{\text{wire}} = \sqrt{\sigma_{\text{B}} \cdot \sigma_{\text{U}}}
\end{equation}
where $\sigma_{\text{B}}$ ($\sigma_{\text{U}}$) is the biased (unbiased) residual width calculated as the amplitude-weighted average of the two Gaussian components, narrow (n) and wide (w), of the fit:
\begin{equation}
\sigma_\alpha = \frac{A_{\text{n}} \cdot \sigma_{\text{n}} + 
                   A_{\text{w}} \cdot \sigma_{\text{w}}}{A_{\text{n}} + A_{\text{w}}}, \hspace{1cm} (\alpha = \text{B}, \ \text{U}).
\end{equation}

The measured single-tube spatial resolutions for all 50 sMDT chambers are shown in Figure~\ref{fig:resolution} for readout thresholds of $-27$~mV and $-39$~mV. The measured resolutions, all below $90~\mu\text{m}$, are well within the specification of $106~\mu\text{m}$. Maintaining a low chamber noise level is essential for achieving optimal tracking resolution.
...
\begin{itemize}
    \item \textbf{Wire tension loss}:  
    The wire tension is set using a calibrated gauge and measured with a custom circuit. During R\&D, the measured tension was $\sim 100~\text{g}$ lower than expected. Investigations excluded gauge calibration and twisting resistance as causes. The issue was traced to the crimping tube not fully seated against the end plug, which shortened the wire by $\sim 100~\mu\text{m}$ during crimping and reduced tension. The solution was to ensure operators held the crimping tube firmly against the end plug during assembly.

    \item \textbf{Tube leakage}:  
    ...
    \item \textbf{Gas-flow hole blockage}:  
    Blockages in the tube gas-flow holes were occasionally observed, caused by debris from the machining process. The problem was mitigated by introducing rigorous cleaning procedures and quality inspections before gas bar assembly.

    \item \textbf{Precision measurement of tube position}:  
...
%%%%%%%%%%%%%%%%%%
Finally, the fully tested sMDT chambers will be integrated with RPC detectors this year and installed in the ATLAS experiment during the LHC Long Shutdown 3 (LS3), beginning at the end of June this year.